...
store_directory = ['/data/retinopathy/OCT/SERI/pre_processed_data/' ...
                   'sankar_2016/'];
...
for idx_file = 1:size(directory_info)
    ...
    if ( ~isempty( strfind( directory_info(idx_file).name, '.img' ) ) )
        ...
        vol_denoised = denoising_volume( vol, method_denoising, ...
                                         kernelratio, windowratio, ...
                                         filterstrength );
        disp( [ 'Image ', directory_info(idx_file).name, ' denoised' ...
                ] );
        [ baseline_vol, vol_flattened ] = flattening_volume( ...
            vol_denoised, method_flattening, 'otsu', false );
